function [cost, grad, cnn] = myCnnBackward(cnn, X, y, theta)

    % myCnnBackward: Implement backpropagation
    % ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    if cnn.layers{numLayers}.softmax == true
        ...
        cost = -1 ./ numImages * groundTruth(:)' * log(cnn.layers{end}.activations(:)); % cross-entrophy loss
        % l2 loss
        for i = 1:numLayers - 1
            cost = cost + l2_penalty * sum(cnn.layers{i}.W(:) .^ 2) + l2_penalty * sum(cnn.layers{i}.b(:) .^ 2);
        end
